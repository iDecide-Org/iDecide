...
const ManageUniversities: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!isAdvisor) {
      console.log(
        "ManageUniversities: Not an advisor or check pending, navigating away."
      ); // <-- Add log here
      navigate("/");
      return;
    }
    ...
    const fetchUniversities = async () => {
      console.log(
        "ManageUniversities: Attempting to fetch advisor universities..."
      ); // <-- Add log here
      setLoading(true);
      try {
        const data = await getAdvisorUniversities();
        console.log(
          "ManageUniversities: Successfully fetched universities:",
          data
        ); // <-- Add log here
